...
describe("CampaignList", () => {
    test("renders table rows correctly", () => {
        ...
        const { getByText, queryAllByText, getAllByRole, queryAllByLabelText } =
            render(<CampaignList campaignData={data} />);

        expect(getByText("Name")).toBeInTheDocument();
        expect(getByText("User Name")).toBeInTheDocument();
        expect(getByText("Start Date")).toBeInTheDocument();
        expect(getByText("End Date")).toBeInTheDocument();
        const activeElements = queryAllByText(
            (content) =>
        ...
        data.forEach((item) => {
            ...
            expect(
                queryAllByText((content) =>
                    item.isActive
                        ? content.includes("Active")
                        : content.includes("Inactive")
                ).length
            ).toBeGreaterThan(0);
            expect(getByText(`${item.Budget}`)).toBeInTheDocument();
        });
    });
});
